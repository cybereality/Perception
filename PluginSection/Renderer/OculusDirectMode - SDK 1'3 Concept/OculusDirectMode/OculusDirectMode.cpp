...
ovrSession *OculusDirectMode::m_phHMD;
					...
					if (bAllCreated)
					{
						...
						if ((m_pbZoomOut) && (eye))
						{
							if (*m_pbZoomOut)
							{
								...
								int index = 0;
								ovr_GetTextureSwapChainCurrentIndex(*m_phHMD, m_psEyeRenderTextureHUD->TextureChain, &index);
								ID3D11Texture2D* tex = nullptr;
								ovr_GetTextureSwapChainBufferDX(*m_phHMD, m_psEyeRenderTextureHUD->TextureChain, index, IID_PPV_ARGS(&tex));
								
								// render by copy recource
								m_pcContextTemporary->CopyResource(tex, m_pcTex11SharedHUD);

								tex->Release();

								m_psEyeRenderTextureHUD->Commit();
							}
						}
					}
				}
