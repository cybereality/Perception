...
void StereoSplitter::Present()
{
	...
	for (DWORD i = 0; i < m_dwNewStereoTwinRenderTargets; i++)
	{
			...
			if (pcStereoTwinSurface == nullptr)
			{
				...
				case D3DRESOURCETYPE::D3DRTYPE_SURFACE:
					...
					if ((desc.Usage & D3DUSAGE_DEPTHSTENCIL) == D3DUSAGE_DEPTHSTENCIL)
					{
						...
						{
							OutputDebugString(L"VireioStereoSplitterDx10 : Failed to create depth stencil surface.");
#ifdef _DEBUG
							wchar_t buf[32];
						...
						if (FAILED(pcDevice->CreateRenderTarget(desc.Width, desc.Height, desc.Format, desc.MultiSampleType, desc.MultiSampleQuality, false, &pcStereoTwinSurface, NULL)))
						{
							...
							{							
								OutputDebugString(L"VireioStereoSplitterDx10 : Failed to create render target.");
#ifdef _DEBUG
								wchar_t buf[32];
								wsprintf(buf, L"desc.Width %u", desc.Width); OutputDebugString(buf);
								wsprintf(buf, L"desc.Height %u", desc.Height); OutputDebugString(buf);
				...
				case D3DRESOURCETYPE::D3DRTYPE_TEXTURE:
					{
						...
						{
							OutputDebugString(L"VireioStereoSplitterDx10 : Failed to create render target texture.");
#ifdef _DEBUG							
							wchar_t buf[32];
							wsprintf(buf, L"desc.Width %u", desc.Width); OutputDebugString(buf);
							wsprintf(buf, L"desc.Height %u", desc.Height); OutputDebugString(buf);
		...
		else
		{
			...
			OutputDebugString(L"VireioStereoSplitterDx10 code failure ! Null pointer monitored render target !");
...
***/
int StereoSplitter::CheckIfMonitored()
{
...
***/
void StereoSplitter::MonitorSurface()
{
