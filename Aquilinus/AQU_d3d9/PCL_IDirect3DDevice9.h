...
File <PCL_IDirect3DDevice9.h> and
Class <PCL_IDirect3DDevice9> :
Copyright © 2014 Denis Reischl
...
#ifndef PCL_IDIRECT3DDEVICE9_CLASS
#define PCL_IDIRECT3DDEVICE9_CLASS
...
/**
* Wrapped IDirect3DDevice9 class, or proxy class.
***/
class PCL_IDirect3DDevice9 : public IDirect3DDevice9
{
public:
	PCL_IDirect3DDevice9(IDirect3DDevice9* pOriginal, AQU_TransferSite* pTransferSite);
	virtual ~PCL_IDirect3DDevice9(void);
...
private:
	/*** PCL_IDirect3DDevice9 private methods ***/
	void DrawIndicator(void);

	/**
	* The actual D3D9 device.
	***/
	...
	* The working area.
	***/
	AQU_WorkingArea *m_pWorkingArea;
	/**
	* The plugin enumeration name vector.
	***/
	std::vector<LPSTR> m_vcPluginNames;
	/**
	* The plugin enumeration file path vector.
	***/
	std::vector<LPWSTR> m_vcPluginFilePathes;
	/**
	* The plugin enumeration type id vector.
	***/
	std::vector<UINT> m_vcPluginIDs;
	/**
	* The plugin enumeration category name.
	***/
	std::vector<LPWSTR> m_vcPluginCategories;
	/**
	* The enlisted plugin prototypes.
	***/
	std::vector<AQU_Nodus*> m_vcPlugins;
	/**
	* The enlisted plugin handles.
	***/
	std::vector<HMODULE> m_vcPluginHandles;
};
